...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Tambah Logik untuk Mengurus Sejarah Gaji (kekal)
    ...
    // Proses upload gambar (kekal sama)
            ...
            if ($_FILES["gambar"]["size"] <= 5000000) {
                if (in_array($imageFileType, ["jpg", "png", "jpeg"])) {
                    if (!empty($anggota['gambar'])) {
                        ...
                        if (file_exists($gambar_lama_path)) unlink($gambar_lama_path);
                    }
    ...
    // SQL untuk kemaskini
    $sql_update = "UPDATE anggota SET nama=?, no_badan=?, kad_pengenalan=?, gaji=?, id_jabatan=?, no_telefon=?, tarikh_masuk_kerja=?, alamat=?, id_jawatan=?, bahagian_id=?, unit_id=?, seksyen_id=?, id_status=?, id_penempatan=?, gambar=? WHERE no_komputer=?";
    $stmt_update = $conn->prepare($sql_update);
    $stmt_update->bind_param("sssdisssiiiiisss", $nama, $no_badan, $kad_pengenalan, $gaji, $id_jabatan, $no_telefon, $tarikh_masuk_kerja, $alamat, $id_jawatan, $bahagian_id, $unit_id, $seksyen_id, $id_status, $id_penempatan, $gambar, $no_komputer);
    
    if ($stmt_update->execute()) {
        // Logik sejarah penempatan (kekal)
        if ($anggota['id_unit_seksyen'] != $seksyen_id || $anggota['id_jawatan'] != $id_jawatan || $anggota['id_status'] != $id_status) {
            ...
            $stmt_sejarah->bind_param("siii", $no_komputer, $seksyen_id, $id_jawatan, $id_status);
        ...
        echo "<script>
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire({icon: 'success', title: 'Berjaya!', text: 'Maklumat anggota berjaya dikemaskini.', confirmButtonText: 'OK'})
                .then(() => window.location.href='./paparan_anggota.php?no_komputer=" . urlencode($no_komputer) . "');
            });
        </script>";
    } else {
        echo "<script>Swal.fire({icon:'error', title:'Ralat!', text:'Ralat semasa mengemaskini maklumat.'});</script>";
    }
    $stmt_update->close();
}
...
<head>
    ...
    <style>
        /* SEMUA STYLE ASAL ANDA KEKAL SAMA */
        ...
        .form-container { max-width: 800px; margin: 80px auto 40px auto; padding: 30px; background-color: white; border-radius: 15px; box-shadow: 0 5px 15px rgba(0,0,0,0.2); border: 2px solid #0d1a40; }
        h2 { color: #0d1a40; text-shadow: 1px 1px 2px rgba(0,0,0,0.1); font-weight: bold; }
        .form-group.row { margin-bottom: 20px; align-items: center; }
        .form-label { font-weight: bold; color: #2c3e50; margin-bottom: 0; text-transform: uppercase; }
        .form-control { border: 2px solid #0d1a40; border-radius: 8px; padding: 10px; width: 100%; text-transform: uppercase; }
        ...
        .current-img { max-width: 200px; margin-top: 10px; border-radius: 10px; border: 2px solid #0d1a40; }
...
<body>
    <div class="header"><h1>SISTEM PENGUATKUASAAN</h1></div>
    <div class="form-container">
        ...
        <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?no_komputer=' . urlencode($no_komputer); ?>" enctype="multipart/form-data">
            <!-- SEMUA MEDAN ASAL KEKAL SAMA -->
            <div class="form-group row">
                <label class="col-sm-4 col-form-label">NO KOMPUTER:</label>
                <div class="col-sm-8"><input type="text" class="form-control" value="<?php echo htmlspecialchars(strtoupper($anggota['no_komputer'])); ?>" disabled></div>
            </div>
            <div class="form-group row">
                <label class="col-sm-4 col-form-label">NAMA:</label>
                <div class="col-sm-8"><input type="text" name="nama" class="form-control" value="<?php echo htmlspecialchars(strtoupper($anggota['nama'])); ?>" required></div>
            </div>
            <div class="form-group row">
                <label class="col-sm-4 col-form-label">NO BADAN:</label>
                <div class="col-sm-8"><input type="text" name="no_badan" class="form-control" value="<?php echo htmlspecialchars(strtoupper($anggota['no_badan'])); ?>"></div>
            </div>
            <div class="form-group row">
                <label class="col-sm-4 col-form-label">KAD PENGENALAN:</label>
                <div class="col-sm-8"><input type="text" name="kad_pengenalan" class="form-control" id="kad_pengenalan" value="<?php echo htmlspecialchars(strtoupper($anggota['kad_pengenalan'])); ?>" placeholder="123456-12-1234"></div>
            </div>
            <div class="form-group row">
                ...
                <div class="col-sm-8">
                    <div class="input-group">
                        ...
                        <input type="number" name="gaji" class="form-control" step="0.01" value="<?php echo htmlspecialchars($anggota['gaji']); ?>">
                    </div>
                </div>
            </div>
            <div class="form-group row">
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-8"><input type="text" name="no_telefon" class="form-control" id="no_telefon" value="<?php echo htmlspecialchars(strtoupper($anggota['no_telefon'])); ?>" placeholder="01X-XXXXXXX"></div>
            </div>
            <div class="form-group row">
                <label class="col-sm-4 col-form-label">TARIKH MASUK KERJA:</label>
                <div class="col-sm-8"><input type="date" name="tarikh_masuk_kerja" class="form-control" value="<?php echo htmlspecialchars($anggota['tarikh_masuk_kerja']); ?>"></div>
            </div>
            <div class="form-group row">
                <label class="col-sm-4 col-form-label">ALAMAT:</label>
                <div class="col-sm-8"><textarea name="alamat" class="form-control"><?php echo htmlspecialchars(strtoupper($anggota['alamat'])); ?></textarea></div>
            </div>

            <!-- BAHAGIAN HIERARKI -->
            <div class="form-group row">
                <label class="col-sm-4 col-form-label">BAHAGIAN:</label>
                <div class="col-sm-8">
                    <select name="bahagian_id" id="bahagian_id" class="form-control" onchange="loadUnit(this.value)" required>
                        ...
                        <?php while ($row = $bahagian_result->fetch_assoc()) { ?>
                            <option value="<?php echo $row['id']; ?>" <?php if ($row['id'] == $anggota['bahagian_id']) echo 'selected'; ?>><?php echo strtoupper($row['nama_bahagian']); ?></option>
                        <?php } ?>
                    </select>
                </div>
            </div>
            ...
            <!-- GAMBAR, JAWATAN, STATUS, PENEMPATAN (kekal) -->
            <div class="form-group row">
    ...
    <script>
        ...
        window.onload = function() {
            const bahagianId = <?php echo $anggota['bahagian_id'] ?? 'null'; ?>;
            if (bahagianId) {
                document.getElementById('bahagian_id').value = bahagianId;
                loadUnit(bahagianId);
            }
        };
    </script>
