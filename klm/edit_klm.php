...
if (empty($id)) {
    echo "<script>alert('ID KLM tidak sah.'); window.location='./senarai_klm.php';</script>";
    exit();
}

// Ambil data sedia ada dari klm_kerja untuk mendapatkan tarikh awal
$sql = "SELECT * FROM klm_kerja WHERE id = ?";
...
if (!$data) {
    echo "<script>alert('Rekod KLM tidak wujud.'); window.location='./senarai_klm.php';</script>";
    exit();
}

// Ambil data anggota
$sql_anggota = "SELECT a.nama, a.id_jawatan, a.id_jabatan, a.gaji, j.nama AS nama_jawatan, d.nama_jabatan AS nama_jabatan 
                FROM anggota a 
                LEFT JOIN jawatan j ON a.id_jawatan = j.id 
                LEFT JOIN jabatan d ON a.id_jabatan = d.id 
                WHERE a.no_komputer = ?";
$stmt_anggota = $conn->prepare($sql_anggota);
$stmt_anggota->bind_param("s", $data['no_komputer']);
$stmt_anggota->execute();
$anggota_result = $stmt_anggota->get_result();
$anggota = $anggota_result->fetch_assoc();
$stmt_anggota->close();

// Ambil gaji dari sejarah_gaji berdasarkan tarikh KLM untuk paparan awal
$tarikh_rujukan = $data['tarikh'];
...
$gaji_query->bind_param("sss", $data['no_komputer'], $tarikh_rujukan, $tarikh_rujukan);
$gaji_query->execute();
$gaji_result = $gaji_query->get_result();
$gaji_papar = $gaji_result->num_rows > 0 ? $gaji_result->fetch_assoc()['gaji'] : 0;
$gaji_query->close();

// Fallback ke gaji dalam table anggota jika tiada rekod dalam sejarah_gaji
if ($gaji_papar == 0) {
    $gaji_papar = $anggota['gaji'] ?? 0;
}

// Proses kemas kini borang
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($no_komputer) || empty($id_jawatan) || empty($kategori_hari) || empty($tarikh) || empty($masa_dari) || empty($masa_hingga) || empty($kenyataan_tugasan)) {
        ...
        // Ambil gaji dari sejarah_gaji berdasarkan tarikh KLM
        $tarikh_rujukan = $tarikh;
        $gaji_query = $conn->prepare("SELECT gaji FROM sejarah_gaji WHERE no_komputer = ? AND tarikh_mula <= ? AND (tarikh_tamat >= ? OR tarikh_tamat IS NULL) ORDER BY tarikh_mula DESC LIMIT 1");
        ...
        // Kemas kini data ke database
        $sql = "UPDATE klm_kerja SET no_komputer=?, id_jawatan=?, id_jabatan=?, kategori_hari=?, tarikh=?, masa_dari=?, masa_hingga=?, jumlah_jam=?, jam_A=?, jam_B=?, jam_C=?, jam_D=?, jam_E=?, gaji_dikira=?, kenyataan_tugasan=? WHERE id=?";
        $stmt = $conn->prepare($sql);
        if ($stmt === false) {
            die("Ralat menyediakan statement: " . $conn->error);
        }
        $stmt->bind_param("siissssdddddddsi", $no_komputer, $id_jawatan, $id_jabatan, $kategori_hari, $tarikh_asal, $masa_dari, $masa_hingga, $jumlah_jam_akhir, $jam_A, $jam_B, $jam_C, $jam_D, $jam_E, $gaji_dikira, $kenyataan_tugasan, $id);
        if ($stmt->execute()) {
            // Simpan log aktiviti
            $log_sql = "INSERT INTO log_aktiviti (aktiviti, tarikh_masa, no_komputer) VALUES (?, NOW(), ?)";
            $log_stmt = $conn->prepare($log_sql);
            $aktiviti = "Mengemas kini rekod KLM (ID: $id) untuk $no_komputer pada tarikh $tarikh_asal";
            $log_stmt->bind_param("ss", $aktiviti, $no_komputer);
            $log_stmt->execute();
            $log_stmt->close();

            echo "<script>alert('Data berjaya dikemas kini!\\n" . $debug_message . "'); window.location='./senarai_klm.php?no_komputer=" . urlencode($no_komputer) . "&bulan=" . urlencode(date('Y-m', strtotime($tarikh))) . "';</script>";
        } else {
            echo "<script>alert('Ralat mengemas kini data: " . $stmt->error . "\\n" . $debug_message . "');</script>";
        }
        $stmt->close();
    }
}
...
<head>
    ...
    <title>KEMAS KINI KLM KERJA - <?php echo htmlspecialchars(strtoupper($anggota['nama'])); ?> - SISTEM PENGUATKUASAAN</title>
    <!-- Bootstrap CSS -->
...
<body>
    ...
    <div class="form-container">
        <h2 class="text-center">KEMAS KINI KLM KERJA - <?php echo htmlspecialchars(strtoupper($anggota['nama'])); ?></h2>
        <form method="POST" action="">
            <input type="hidden" name="no_komputer" value="<?php echo htmlspecialchars($data['no_komputer']); ?>">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" value="<?php echo htmlspecialchars(strtoupper($anggota['nama'])); ?>" readonly>
            </div>
            <div class="mb-3">
                <label class="form-label">NO KOMPUTER:</label>
                <input type="text" class="form-control" value="<?php echo htmlspecialchars($data['no_komputer']); ?>" readonly>
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" value="<?php echo 'RM' . number_format($gaji_papar, 2); ?>" readonly>
            </div>
            <div class="mb-3">
                <label class="form-label">KATEGORI HARI:</label>
                <select name="kategori_hari" class="form-control" required>
                    <option value="BIASA" <?php echo $data['kategori_hari'] == 'BIASA' ? 'selected' : ''; ?>>BIASA</option>
                    <option value="MINGGU" <?php echo $data['kategori_hari'] == 'MINGGU' ? 'selected' : ''; ?>>MINGGU</option>
                    <option value="KELEPASAN" <?php echo $data['kategori_hari'] == 'KELEPASAN' ? 'selected' : ''; ?>>KELEPASAN</option>
                    <option value="PERISTIWA" <?php echo $data['kategori_hari'] == 'PERISTIWA' ? 'selected' : ''; ?>>PERISTIWA</option>
                </select>
            </div>
            <div class="mb-3">
                <label class="form-label">TARIKH:</label>
                <input type="date" name="tarikh" class="form-control" value="<?php echo htmlspecialchars($data['tarikh']); ?>" required>
            </div>
            <div class="mb-3">
                <label class="form-label">MASA DARI:</label>
                <input type="time" name="masa_dari" class="form-control" value="<?php echo htmlspecialchars($data['masa_dari']); ?>" required>
            </div>
            <div class="mb-3">
                <label class="form-label">MASA HINGGA:</label>
                <input type="time" name="masa_hingga" class="form-control" value="<?php echo htmlspecialchars($data['masa_hingga']); ?>" required>
            </div>
            <div class="mb-3">
                <label class="form-label">KENYATAAN TUGASAN:</label>
                <textarea name="kenyataan_tugasan" class="form-control" required><?php echo htmlspecialchars($data['kenyataan_tugasan']); ?></textarea>
            </div>
            <div class="btn-group">
                <button type="submit" class="btn btn-primary btn-custom"><i class="bi bi-save me-2"></i>KEMAS KINI</button>
                <a href="./senarai_klm.php?no_komputer=<?php echo urlencode($data['no_komputer']); ?>&bulan=<?php echo urlencode(date('Y-m', strtotime($data['tarikh']))); ?>" class="btn btn-secondary btn-custom"><i class="bi bi-arrow-left me-2"></i>KEMBALI</a>
